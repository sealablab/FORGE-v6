#!/usr/bin/env python3
"""
CocoTB Python Test Runner for BPD Component Tests

STATUS: Archived after HVS migration (2025-11-11)

This runner previously supported fsm_observer component tests, which were removed
...
This runner can be reused for future BPD component tests by adding configurations
to test_configs.py following the forge-vhdl test infrastructure pattern.

Legacy Usage (when tests configured):
    python run.py <test_name>              # Run single test (P1 default)
    TEST_LEVEL=P2 python run.py <test>     # Run P2 tests
    python run.py --list                   # List available tests

Author: BPD Development Team (adapted from forge-vhdl runner)
Date: 2025-11-11
"""
