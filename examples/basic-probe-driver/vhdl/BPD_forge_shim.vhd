...
architecture rtl of BPD_forge_shim is
    ...
    ----------------------------------------------------------------------------
    -- FSM State Signals (for HVS hierarchical encoder)
    ----------------------------------------------------------------------------
    signal current_state        : std_logic_vector(5 downto 0);  -- Raw 6-bit FSM state
    signal current_status       : std_logic_vector(7 downto 0);  -- 8-bit status byte
...
begin
    ...
    BPD_MAIN_INST: entity WORK.BPD_forge_main
        ...
        port map (
            ...
            -- FSM State Output (raw 6-bit state + 8-bit status for HVS encoder)
    ...
    HVS_ENCODER: entity WORK.forge_hierarchical_encoder
        ...
        port map (
            clk           => Clk,
            reset         => Reset,
            state_vector  => current_state,       -- 6-bit FSM state (0-63)
            status_vector => current_status,      -- 8-bit status byte
            voltage_out   => hvs_encoded_voltage  -- Signed 16-bit digital output
        );

    ----------------------------------------------------------------------------
    -- OutputC Mapping: HVS Hierarchical Encoded State+Status
    -- Digital encoding: state × 200 + status_offset
    -- Oscilloscope will show voltage proportional to digital value
    ----------------------------------------------------------------------------
    OutputC <= hvs_encoded_voltage;
