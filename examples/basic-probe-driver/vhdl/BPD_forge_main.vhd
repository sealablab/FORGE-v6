...
entity BPD_forge_main is
    ...
    port (
        ...
        ------------------------------------------------------------------------
        -- FSM State Output (for HVS voltage encoding in shim layer)
        -- Raw 6-bit state vector + 8-bit status for forge_hierarchical_encoder
        ------------------------------------------------------------------------
        current_state  : out std_logic_vector(5 downto 0);  -- 6-bit FSM state
        current_status : out std_logic_vector(7 downto 0)   -- 8-bit status byte
                                                             -- status[7] = fault flag
                                                             -- status[6:0] = app status
    );
...
architecture rtl of BPD_forge_main is
    ...
    ----------------------------------------------------------------------------
    -- FSM State Constants (6-bit encoding for HVS hierarchical encoder)
    ----------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------------
    -- Status Byte Encoding (for HVS hierarchical encoder)
    -- status[7]   = fault flag (1 = FAULT state, negative voltage)
    -- status[6:0] = application status (0-127 range for fine-grained info)
    ----------------------------------------------------------------------------
...
begin
    ...
    -- Status Byte Encoding (for HVS hierarchical encoder)
    --
    -- Encodes application status into lower 7 bits with fault flag in bit 7
    -- This provides fine-grained debugging information beyond just state
    ----------------------------------------------------------------------------
    STATUS_ENCODER: process(state, trig_out_active, intensity_active,
                           monitor_window_open, timeout_occurred, firing_complete)
        variable status_lower : std_logic_vector(6 downto 0);
    begin
        -- Encode application status in lower 7 bits
        -- Bit allocation:
        --   status[0] = trig_out_active
        --   status[1] = intensity_active
        --   status[2] = monitor_window_open
        --   status[3] = timeout_occurred
        --   status[4] = firing_complete
        --   status[6:5] = reserved (0)
        status_lower := (
            0 => trig_out_active,
            1 => intensity_active,
            2 => monitor_window_open,
            3 => timeout_occurred,
            4 => firing_complete,
            others => '0'
        );

        -- status[7] = fault flag (1 when in FAULT state)
        if state = STATE_FAULT then
            status_byte <= '1' & status_lower;  -- Fault: negative voltage in HVS
        else
            status_byte <= '0' & status_lower;  -- Normal: positive voltage in HVS
        end if;
    end process;
    ...
    -- FSM state and status outputs (for HVS hierarchical encoder in shim)
    current_state  <= state;
